...
public class ColheitaRepository : IColheitaRepository
{
  ...
  public ColheitaRepository(RaizesDbContext ctx)=>_ctx=ctx;

  public async Task AddAsync(Colheita entity){
    _ctx.Add(entity);
    await _ctx.SaveChangesAsync();
  }
  public Task<> GetByIdAsync(Guid id)=>_ctx.Set<Colheita>().FindAsync(id).AsTask();
  public async Task UpdateAsync(Colheita entity){
    _ctx.Update(entity);
    await _ctx.SaveChangesAsync();
  }
