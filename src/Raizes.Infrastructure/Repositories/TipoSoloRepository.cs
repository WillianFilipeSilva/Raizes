...
public class TipoSoloRepository : ITipoSoloRepository
{
  ...
  public TipoSoloRepository(RaizesDbContext ctx)=>_ctx=ctx;

  public async Task AddAsync(TipoSolo entity){
    _ctx.Add(entity);
    await _ctx.SaveChangesAsync();
  }
  public Task<> GetByIdAsync(Guid id)=>_ctx.Set<TipoSolo>().FindAsync(id).AsTask();
  public async Task UpdateAsync(TipoSolo entity){
    _ctx.Update(entity);
    await _ctx.SaveChangesAsync();
  }
