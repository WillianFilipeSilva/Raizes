...
public class PropriedadeRepository : IPropriedadeRepository
{
  ...
  public PropriedadeRepository(RaizesDbContext ctx)=>_ctx=ctx;

  public async Task AddAsync(Propriedade entity){
    _ctx.Add(entity);
    await _ctx.SaveChangesAsync();
  }
  public Task<> GetByIdAsync(Guid id)=>_ctx.Set<Propriedade>().FindAsync(id).AsTask();
